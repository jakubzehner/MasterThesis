...
class CcAutoRegressiveSeqDecoder(SeqDecoder):
    ...
    def __init__(
        ...
        # Used for expanded (like cat) decoder output
        target_projecting_input_dim: Optional[int] = None,
        **kwargs
    ) -> None:
        ...
        self._start_index = self._vocab.get_token_index(
            START_SYMBOL, self._target_namespace) if msg_start_token_index is None else msg_start_token_index
        self._end_index = self._vocab.get_token_index(
            END_SYMBOL, self._target_namespace) if msg_end_token_index is None else msg_end_token_index
        ...
        target_vocab_size = self._vocab.get_vocab_size(
            self._target_namespace) if target_vocab_size is None else target_vocab_size
        ...
        if copy_mode != 'none':
            self.copy_net = FiraCopyNet(
                encoder_output_dim, output_projecting_dim, copynet_hidden_dim)
    ...
    def maybe_expand_decoder_output_with_fusion(self,
        ...
        if 'fused_cat_context' in state:
            ...
            if decoder_output.ndim == 3:
                ...
                cc_fused_features = cc_fused_features.unsqueeze(
                    1).expand(-1, seq_len, -1)
            decoder_output = torch.cat(
                (decoder_output, cc_fused_features), dim=-1)
    ...
    def _forward_loss(
        ...
        if self._scheduled_sampling_ratio == 0 and self._decoder_net.decodes_parallel:
            ...
            decoder_output = self.maybe_expand_decoder_output_with_fusion(
        ...
        if self.copy_mode == 'copy_by_id':
            ...
            if not self.training:
                self.update_copy_stat('copy_label_count',
                                      copy_mask.sum().item())

            copy_logits, gate = self.copy_net(
                encoder_outputs, decoder_output, source_mask)
            log_probs = compute_copy_extended_log_probs(
                logits, copy_logits, gate)
        ...
        output_dict = {"loss": torch.nan_to_num(loss, 20)}
    ...
    def take_step_with_copy(
        ...
        rev_last_predictions, _ = revert_extend_predictions_by_id(
            last_predictions.unsqueeze(-1), source_token_ids, self.target_vocab_size)
        ...
        copy_logits, gate = self.copy_net(
            encoder_outputs, decoder_step_output.unsqueeze(1), source_mask)
        # shape: (group_size, num_classes + extend_size)
        class_log_probabilities = compute_copy_extended_log_probs(
            output_projections.unsqueeze(1), copy_logits, gate)
    ...
    def forward(
        ...
        if target_tokens:
            state_forward_loss = (
                state if self.training else {
                    k: v.clone() for k, v in state.items()}
            )
            output_dict = self._forward_loss(
                state_forward_loss, target_tokens, source_tokens)
        else:
            output_dict = {}

        if not self.training:
            source_token_ids = util.get_token_ids_from_text_field_tensors(
                source_tokens)
            state.update({
            ...
            if target_tokens:
                ...
                if self._tensor_based_metric is not None:
                    ...
                    if self.copy_mode == 'copy_by_id':
                        reverted_best_predictions, copy_count = revert_extend_predictions_by_id(
                            best_predictions, source_token_ids, self.target_vocab_size)
                    ...
                    self._tensor_based_metric(
                        best_predictions, targets)  # type: ignore
    ...
    def post_process(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.copy_mode != 'none':
            raise NotImplementedError(
                f"post_process method has not been adapted for copy_mode={self.copy_mode}")

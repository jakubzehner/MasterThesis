...
class TransformerDecoderNet(DecoderNet):
    ...
    def __init__(
        ...
        self.target_input_linear = torch.nn.Linear(
            target_embedding_dim, decoding_dim)
        ...
        self._self_attention = Decoder(
            DecoderLayer(
                decoding_dim, deepcopy(attn), deepcopy(
                    attn), feed_forward, residual_dropout_prob
            ),
            num_layers,
        )
    ...
    def forward(
        ...
        # Project target embedding to the same decoding dim first.
        previous_steps_predictions = self.target_input_linear(
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        x = self.sublayer[1](x, lambda y: self.src_attn(
            y, memory, memory, src_mask))
